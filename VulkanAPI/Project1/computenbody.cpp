...
public:
	...
	void loadAssets()
	{
		textures.particle.loadFromFile("textures/particle01_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
		textures.gradient.loadFromFile("textures/particle_gradient_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
	}
	...
	void prepareStorageBuffers()
	{
		...
		VulkanBase::flushCommandBuffer(copyCmd, queue, true);
	...
	void preparePipelines()
	{
		...
		shaderStages[0] = loadShader("shaders/computenbody/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader("shaders/computenbody/particle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		// 1st pass
		computePipelineCreateInfo.stage = loadShader("shaders/computenbody/particle_calculate.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		...
		// 2nd pass
		computePipelineCreateInfo.stage = loadShader("shaders/computenbody/particle_integrate.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
...
int main(const int argc, const int *argv[]) {
	VulkanExample *vulkanExample; //Derived from VulkanBase in your main program. EX: class VulkanProgram : public VulkanBase {}
